...
uses
  LCLIntf, LResources,  Messages, Forms, SysUtils, Classes,
  Graphics, Controls, StdCtrls, ExtCtrls,  Menus, FileUtil, DAVDCommon,
  DVstEffect, WaveIOX, MIDIFile, MIDI, DVSTHost, DASIOHost;
...
uses
  ShellAPI,
  Math, Inifiles, Dialogs, OptionsForm, AboutForm, PlayerForm;
...
{$ENDIF}
 ...
 if (MidiFile.filename <> '') and FileExistsUTF8(MidiFile.filename) { *Konvertiert von FileExists* }
 ...
 if (ParamCount > 0) and (FileExistsUTF8(Paramstr(1)) { *Konvertiert von FileExists* }) then
  begin
   LoadPlugin(Paramstr(1));
   VSTHost[0].OnProcessEvents := ProcessEvents;
  end
 else if FileExistsUTF8(s) { *Konvertiert von FileExists* } then
  begin
   fLoadProg := i;
   LoadPlugin(s, i);
   VSTHost[0].OnProcessEvents := ProcessEvents;
...
begin
 WaveFile.unload;
 if FileExistsUTF8(fn) { *Konvertiert von FileExists* } then
 begin
  WaveFile.load(fn);
...
var r: ERect;
    i: Integer;
begin
 if not FileExistsUTF8(s) { *Konvertiert von FileExists* } then exit;
 WaveTimer.enabled := False;
 fProcessing := False;
 StopAudio;
 sleep(2);
...
begin
 if not FileExistsUTF8(Wavefile.filename) { *Konvertiert von FileExists* } then exit;
 WaveFile.play;
end;
