...
{$endif}
  ...
  function IsInheritedFrom(Objtype:TClass;className:shortString):Boolean;
var
...
begin
 Result := false;
 Result := SameText(objType.ClassName, className);
 if not Result then
    if Objtype.ClassParent<>nil then
    begin
       Result := IsInheritedFrom(Objtype.ClassParent,className);
    end;
end;
