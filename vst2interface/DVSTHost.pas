...
type
  ...
  public
    ...
    function GetProgramNameIndexed(category, index: Integer; ProgramName: PAnsiChar): Integer;
  ...
  published
    ...
    property uID: Ansistring read FuID stored False;
...
begin
 ...
 if FourCharToLong(PVstEffect.Magic[3],PVstEffect.Magic[2],PVstEffect.Magic[1],PVstEffect.Magic[0])<>FourCharToLong('V','s','t','P')
...
function TVstPlugin.GetProgramNameIndexed(category, index: Integer; ProgramName: PAnsiChar): Integer;
begin
 if FActive then
 ...
 case FDLLHandle = 0 of
   ...
   try
    ...
    if Buf='' then
     begin
      str:=IntToStr(LE);
      StrCopy(PansiChar(@Buf[0]),PansiChar(@str[1]));
    ...
    begin
     ...
     begin
        @FMainFunction := GetProcAddress(FDLLHandle, 'VSTPluginMain');
     end;
     if not Assigned(FMainFunction) then
      begin
...
procedure Register;
begin
  RegisterComponents('VST Interface', [TVstHost]);
end;
