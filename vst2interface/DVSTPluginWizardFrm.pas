...
{$ENDIF}
  Messages, Classes, Controls, Forms, StdCtrls, ComCtrls, ExtCtrls,
  Config, Graphics, FileUtil;
  ...
  case StepIndex of
    STEP_DESTINATION:
      begin
        ...
        else if not DirectoryExistsUTF8(edtProjectPath.Text) { *Konvertiert von DirectoryExists* } then
        begin
          if MessageDlg(PROJECT_PATH_DOES_NOT_EXIST, mtConfirmation, [mbYes,
            mbNo], 0) = mrYes then
          begin
            ForceDirectoriesUTF8(edtProjectPath.Text); { *Konvertiert von ForceDirectories* }
            Result := True;
          end;
        end
        else
