...
uses Registry, ComObj,Windows, Math {$IFDEF ASIOMixer}, ASIOChannelStrip {$ENDIF};

procedure Register;
begin
  RegisterComponents('asioInterface',[TASIOHost]);
end;
...
begin
 ...
 for i := 0 to FInputChannelCount - 1 do
  begin
   ...
   FDriver.GetChannelInfo(FInputChannelInfos[i]);
    case FInputChannelInfos[i].vType of
 ...
 for i := 0 to FOutputChannelCount - 1 do
  begin
   ...
   case FOutputChannelInfos[i].vType of
...
procedure TCustomASIOHostBasic.GetWinowHandle;
var
...
begin
  ...
  FHandle := CreateWindowEx(WS_EX_TOOLWINDOW, UtilWindowClass.lpszClassName,
    '', WS_POPUP {+ 0}, 0, 0, 0, 0, 0, 0, HInstance, nil);
    SetWindowLongPtr(FHandle, GWL_WNDPROC, IntPtr(MakeObjectInstance(WndProc)));
end;

procedure TCustomASIOHostBasic.freeWindowHandle;
begin
...
procedure TCustomASIOHostBasic.GetOutputGain(Channel, Gain: Integer);
var ACC : TASIOChannelControls;
begin
 if FDriver = nil then Exit;
