...
begin
  ...
  if(mmioRead(mm,PAnsiChar(@FormatTmp),16)<>16) then
  ...
  if(ExtraAlloc<>0) then
    if(mmioRead(mm,PAnsiChar(FFormat)+sizeof(FFormat^),ExtraAlloc) <>
      ExtraAlloc) then
    ...
    if(FPosition>=PCMBufferSamplePos) and(FPosition<PCMBufferSamplePos+PCMBufferSampleSize) // use current buffer data if possible
     then
      begin
       ...
       CopyMemory(PAnsiChar(@Buffer), PAnsiChar(PCMBuffer)+(FPosition-PCMBufferSamplePos)*dstwfx^.nBlockAlign, len*dstwfx^.nBlockAlign);
       pos:=len;
      end
     else pos:=0;
